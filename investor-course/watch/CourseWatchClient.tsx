    ...
    <>
      ...
      <main style={{ background:"var(--dark)",minHeight:"100vh",paddingTop:"5.5rem" }}>
        ...
        <div style={{ maxWidth:900,margin:"0 auto",padding:"3rem 2rem 6rem" }}>
          <h1 style={{ fontFamily:"'Cormorant Garamond', Georgia, serif",fontSize:"2rem",fontWeight:300,color:"white",marginBottom:"0.5rem" }}>Kentucky Real Estate Investor Course</h1>
                ...
                {mod.lessons.map((lesson,i) => {
                  ...
                  return (
                    <div key={k} className="lr" onClick={()=>toggle(k)} role="button" tabIndex={0} onKeyDown={(e)=>e.key==="Enter"&&toggle(k)}>
                      ...
                      <Link href={`/investor-course/watch/${mod.slug}#lesson-${i}`} style={{ fontSize:"0.88rem",color:d?"var(--muted2)":"#C8C4BC",textDecoration:d?"line-through":"none",flex:1 }} onClick={(e)=>e.stopPropagation()}>{lesson}</Link>
                    </div>
                  );
                })}
              </div>

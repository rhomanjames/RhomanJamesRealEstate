          ...
          <div className="tool-inputs">
            ...
            <div style={{ marginTop: "2rem", padding: "1rem", background: "var(--border)", border: "1px solid var(--border)", borderRadius: "16px", boxShadow: "none" }}>
              ...
              <p style={{ fontSize: "0.78rem", color: "var(--muted)", lineHeight: 1.7 }}>
                ...
                <strong style={{ color: "var(--text)" }}>USDA Loan:</strong> Zero down for eligible rural properties in Hardin County.<br />
                <strong style={{ color: "var(--text)" }}>KHC Assistance:</strong> Up to $10,000 in down payment assistance for qualifying KY buyers.
              </p>
            </div>
          </div>

          <div className="tool-results">
            <div style={{ fontSize: "0.62rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--blue)", marginBottom: "1rem" }}>Time to Reach Each Target</div>
            {targets.map((t) => {
              ...
              return (
                <div key={t.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1rem", marginBottom: "0.75rem", background: reached ? "var(--border)" : "rgba(255,255,255,0.02)", border: `1px solid ${reached ? "var(--border)" : "var(--border)"}` }}>
                  <div>
                    <div style={{ fontSize: "0.78rem", color: reached ? "var(--blue)" : "var(--text)", marginBottom: "0.2rem" }}>{t.label}</div>
                    <div style={{ fontSize: "1.1rem", fontFamily: "'Inter', system-ui, sans-serif", color: "var(--muted)" }}>{fmt(targetAmt)}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: "1.1rem", fontFamily: "'Inter', system-ui, sans-serif", color: reached ? "var(--blue)" : "var(--text)" }}>{timeStr}</div>
            ...
            <div style={{ marginTop: "1rem", padding: "1rem", background: "rgba(255,255,255,0.02)", border: "1px solid var(--border)", borderRadius: "16px", boxShadow: "none" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
                ...
                <div><div style={{ fontSize: "0.65rem", color: "var(--muted2)" }}>Saving per year</div><div style={{ fontSize: "0.95rem", color: "var(--text)", fontFamily: "'Inter', system-ui, sans-serif" }}>{fmt(monthlySavings * 12)}</div></div>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div style={{ marginTop: "1.5rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "0.25rem" }}>
        ...
        <span style={{ fontSize: "0.85rem", color: "var(--blue)", fontFamily: "'Inter', system-ui, sans-serif" }}>{display}</span>
      </div>

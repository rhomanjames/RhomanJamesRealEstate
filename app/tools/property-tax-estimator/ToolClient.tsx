      ...
      <ToolLayout num="09" title="Property Tax Estimator" description="Estimate annual and monthly property taxes for homes in Hardin, LaRue, and Meade counties — with Kentucky homestead exemption included.">
        <div className="tool-grid">
          <div className="tool-inputs">
            ...
            <div style={{ marginTop: "1.5rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", marginTop: "0.5rem" }}>
                {counties.map((c, i) => (
                  <button key={c.name} onClick={() => { setCountyIdx(i); setCityIdx(0); }} style={{ padding: "0.65rem 1rem", textAlign: "left", border: `1px solid ${countyIdx === i ? "var(--gold)" : "rgba(184,151,106,0.2)"}`, background: countyIdx === i ? "rgba(184,151,106,0.12)" : "transparent", color: countyIdx === i ? "var(--gold)" : "var(--muted)", cursor: "pointer", fontSize: "0.78rem", fontFamily: "'Jost', system-ui, sans-serif", display: "flex", justifyContent: "space-between" }}>
                    <span>{c.name}</span>
            ...
            <div style={{ marginTop: "1.5rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", marginTop: "0.5rem" }}>
                {county.cities.map((c, i) => (
                  <button key={c.name} onClick={() => setCityIdx(i)} style={{ padding: "0.6rem 1rem", textAlign: "left", border: `1px solid ${cityIdx === i ? "var(--gold)" : "rgba(184,151,106,0.2)"}`, background: cityIdx === i ? "rgba(184,151,106,0.12)" : "transparent", color: cityIdx === i ? "var(--gold)" : "var(--muted)", cursor: "pointer", fontSize: "0.78rem", fontFamily: "'Jost', system-ui, sans-serif", display: "flex", justifyContent: "space-between" }}>
                    <span>{c.name}</span>
            ...
            <div style={{ marginTop: "1.5rem", display: "flex", alignItems: "center", gap: "0.75rem" }}>
              <input type="checkbox" id="homestead" checked={homestead} onChange={e => setHomestead(e.target.checked)} style={{ accentColor: "var(--gold)", width: 16, height: 16, cursor: "pointer" }} />
          ...
          <div className="tool-results">
            <div style={{ padding: "1.75rem", background: "rgba(184,151,106,0.06)", border: "1px solid rgba(184,151,106,0.25)", textAlign: "center", marginBottom: "1.25rem" }}>
            ...
            <div style={{ fontSize: "0.62rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--gold)", marginBottom: "0.75rem" }}>Tax Breakdown</div>
            {[
              ...
              <div key={item.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", padding: "0.65rem 0", borderBottom: "1px solid rgba(184,151,106,0.08)" }}>
                <div>
                ...
                <span style={{ fontSize: "0.9rem", color: "var(--off-white)", fontFamily: "'Cormorant Garamond', Georgia, serif" }}>{fmt(item.amt)}/yr</span>
              </div>
            ))}

            {homestead && (
              <div style={{ marginTop: "0.75rem", padding: "0.75rem", background: "rgba(184,151,106,0.06)", border: "1px solid rgba(184,151,106,0.2)", fontSize: "0.75rem", color: "var(--muted)" }}>

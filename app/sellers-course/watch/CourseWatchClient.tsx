                ...
                {mod.lessons.map((lesson,i) => {
                  ...
                  return (
                    <div key={k} className="lr" onClick={()=>toggle(k)} role="button" tabIndex={0} onKeyDown={(e)=>e.key==="Enter"&&toggle(k)}>
                      ...
                      <Link href={`/sellers-course/watch/${mod.slug}#lesson-${i}`} style={{ fontSize:"0.88rem",color:d?"var(--muted2)":"#C8C4BC",textDecoration:d?"line-through":"none",flex:1 }} onClick={(e)=>e.stopPropagation()}>{lesson}</Link>
                    </div>
                  );
                })}
              </div>

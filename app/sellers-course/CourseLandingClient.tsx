  ...
  return (
    <main>
      <section style={{ paddingTop: "8rem", paddingBottom: "5rem", paddingLeft: "1.5rem", paddingRight: "1.5rem", background: "var(--bg-soft)", position: "relative", textAlign: "center" }}>
        ...
        <h1 style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: "clamp(2.5rem, 6vw, 4.5rem)", fontWeight: 700, color: "var(--text)", lineHeight: 1.1, marginBottom: "1.25rem" }} dangerouslySetInnerHTML={{ __html: `The Kentucky<br /><em style="font-style:italic;color:var(--blue)">Home Sellers Course</em>` }} />
        ...
        <div style={{ maxWidth: 480, margin: "0 auto", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "16px", padding: "2.5rem" }}>
          ...
          <form onSubmit={async (e) => { e.preventDefault(); const form = e.currentTarget; const data = new FormData(form); try { await fetch("https://formspree.io/f/xpqgrodo", { method: "POST", body: data, headers: { Accept: "application/json" } }); } catch(err) {} window.location.href = "/sellers-course/watch"; }} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            <input type="hidden" name="_subject" value="New Sellers Course Sign-Up" />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem" }}>
              <label htmlFor="timeline" style={{ fontSize: "0.62rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--muted2)", textAlign: "left" }}>When are you thinking of selling?</label>
              <select className="form-input" id="timeline" name="timeline" style={{ appearance: "none" }}>
                ...
                <option>Just curious about the process</option>
              </select>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem" }}>
              <label htmlFor="neighborhood" style={{ fontSize: "0.62rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--muted2)", textAlign: "left" }}>What area or neighborhood is your home in?</label>
              <input className="form-input" type="text" id="neighborhood" name="neighborhood" placeholder="e.g. Elizabethtown, Radcliff, Vine Grove..." />
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem" }}>
              <label htmlFor="agent" style={{ fontSize: "0.62rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--muted2)", textAlign: "left" }}>Are you currently working with an agent?</label>
              <select className="form-input" id="agent" name="agent" style={{ appearance: "none" }}>
                <option value="">Select one</option>
                <option>No, I am looking</option>
                <option>Yes, but exploring my options</option>
                <option>Not sure yet</option>
              </select>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem" }}>
              <label htmlFor="reason" style={{ fontSize: "0.62rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--muted2)", textAlign: "left" }}>Primary reason for selling</label>
              <select className="form-input" id="reason" name="reason" style={{ appearance: "none" }}>
                <option value="">Select one</option>
                <option>Upsizing</option>
                <option>Downsizing</option>
                <option>Relocating out of area</option>
                <option>PCS / Military move</option>
      ...
      <section style={{ background: "var(--bg-card)", padding: "6rem 1.5rem", textAlign: "center", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <h2 style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: "clamp(2rem, 4vw, 3rem)", fontWeight: 700, color: "var(--text)", marginBottom: "1rem", lineHeight: 1.2 }} dangerouslySetInnerHTML={{ __html: `Ready to sell for<br /><em style="font-style:italic;color:var(--blue)">top dollar in Kentucky?</em>` }} />
